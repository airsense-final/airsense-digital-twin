...
// --- 3D BOX COMPONENT (Sensor) ---
const SensorNode = ({ data }: { data: TwinSensor }) => {
  // Alarm Logic: Red if value exceeds 100, otherwise Green
...
export const FactoryScene = () => {
  ...
  // Write the Token key from your own system here!
  // If it is 'token' in localStorage, use 'token'. If 'access_token', use that.
  const token = localStorage.getItem('access_token') || localStorage.getItem('token'); 

  // TASK 1 (Task 5): Fetch Company List and Populate Dropdown
  useEffect(() => {
    // Python Port: 8001
    axios.get('http://127.0.0.1:8001/api/companies')
      .then(res => {
         setCompanies(res.data);
         // Automatically select the first company if the list has any (So the user doesn't see an empty screen)
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      {/* --- TOP PANEL (TASK 5: Company Selector) --- */}
      <div style={{ 
        background: '#1f2937', 
        padding: '15px 20px', 
        borderRadius: '12px', 
      ...
      {/* --- BOTTOM PANEL (3D Scene - To be detailed in Task 6) --- */}
      <div style={{ 
        width: '100%', 
        height: '600px', 
        background: '#111827', 
        ...
        <Canvas camera={{ position: [20, 30, 30], fov: 45 }}>
          ...
          {/* Sensors (Empty for now, will be filled in Task 6-7) */}
